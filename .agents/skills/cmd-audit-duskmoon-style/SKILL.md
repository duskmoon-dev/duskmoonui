...
Detected by cmd-audit-duskmoon-style"
```
...
**Alternative: Interactive merge with the `$cmd-worktree-merge` skill**
```bash
...
> $cmd-audit-duskmoon-style

# Run audit and create worktrees for fixing
> $cmd-audit-duskmoon-style --fix

# Run audit, fix, and merge all in one go
> $cmd-audit-duskmoon-style --fix --merge
