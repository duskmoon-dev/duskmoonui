...
Native `details[open]` is authoritative: `.open` and `.accordion-item-open` do
not override native open/closed state. Each item can expand independently.

<ComponentShowcase title="Native disclosure" code={`<div class="accordion">
  <details class="accordion-item" name="demo-delivery">
    <summary class="accordion-header">Delivery</summary>
    <div class="accordion-content"><div class="accordion-body"><div class="accordion-body-inner">Ships in two days.</div></div></div>
  </details>
  <details class="accordion-item" name="demo-delivery">
    <summary class="accordion-header">Returns</summary>
    <div class="accordion-content"><div class="accordion-body"><div class="accordion-body-inner">Returns within 30 days.</div></div></div>
  </details>
</div>`} />

Shared `name` makes an exclusive native group in browsers supporting grouped
details. Without that support items remain independent; if exclusivity is
required, provide an application controller. Omit `name` for independent items.

## Controlled Disclosure

<DataDisplayDemo kind="disclosure" />

For controlled markup, `.open` is canonical and `.accordion-item-open` remains
a compatibility alias. Classes only style the owning item's direct panel.
CSS does not update ARIA or move focus. Initialize closed panels with `hidden`
and `inert`; keep the trigger outside the panel. Use a unique panel ID.

```js
function setExpanded(item, expanded) {
  const trigger = item.querySelector(':scope > .accordion-header');
...
// Bind trigger clicks. For exclusive groups, close direct sibling items using
// the same setter before opening this item. Do not select nested items.
```

Restore focus to the trigger before hiding a panel containing focus. Preserve
normal Tab navigation rather than trapping focus. The older framework/class
examples below illustrate composition; controllers must implement this complete
state contract, including `hidden`/`inert` on initial render.
...
function toggleAccordion(button) {
  ...
  const expanded = !item.classList.contains('open') && !item.classList.contains('accordion-item-open');
  const set = (target, open) => {
    const trigger = target.querySelector(':scope > .accordion-header');
    const panel = target.querySelector(':scope > .accordion-content');
    if (!open && panel.contains(document.activeElement)) trigger.focus();
    panel.id ||= 'demo-panel-' + crypto.randomUUID();
    trigger.setAttribute('aria-controls', panel.id);
  ...
  if (isSingle && expanded) accordion.querySelectorAll(':scope > .accordion-item:not(details)').forEach(other => {
    if (other !== item) set(other, false);
  });
  set(item, expanded);
}
</script>`} />
...
function AccordionItem({ title, children, icon }: AccordionItemProps) {
  ...
  return (
    <div className={`accordion-item ${isOpen ? 'accordion-item-open' : ''}`}>
      ...
      <div className="accordion-content" id={panelId} hidden={!isOpen} inert={!isOpen}>
...
<template>
  <div :class="itemClasses">
    ...
    <div class="accordion-content" :id="panelId" :hidden="!isOpen" :inert="!isOpen">
      <div class="accordion-body">
        <slot />
      </div>
    </div>
...
- [List](/duskmoonui/docs/en/components/list/) - List display component
