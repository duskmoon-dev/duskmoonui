...
description: Mobile bottom navigation, also known as a dock, with semantic current-page states
category: components
order: 20
published: true
tags: ['bottom-navigation', 'dock', 'navigation', 'components', 'material-design', 'mobile']
---
import ComponentShowcase from '../../../../components/showcase/ComponentShowcase.astro';

# Bottom Navigation (Dock)

Bottom navigation bars, often called **docks**, let users move between an app's primary destinations. Use anchors for destinations and mark the current one with `aria-current="page"`. The legacy `active` and `bottom-nav-item-active` classes remain supported for compatibility.

The component is fixed by default. Reserve at least its height plus `env(safe-area-inset-bottom)` as padding at the end of the application's scrollable content so the dock does not cover the final controls.
...
Unavailable destinations should not be links. Render a disabled button for an unavailable action, or plain text for a destination that cannot be visited:

<ComponentShowcase title="Disabled Item" code={`<button type="button" class="bottom-nav-item bottom-nav-item-disabled" disabled>
  <svg class="bottom-nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
  </svg>
...
| `.bottom-nav-shifting` | Animated shifting layout; preferred spelling for new code |
...
`.bottom-nav-shift` is retained as the legacy spelling. Prefer `.bottom-nav-shifting` for new animated shifting layouts.
...
- [Color System](/duskmoonui/docs/en/theme/colors/)
