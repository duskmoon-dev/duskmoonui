...
## Compatibility And Responsive Composition

Both `table-striped` and `table-zebra` are shipped. Selected rows support
`table-row-selected` and `selected`; selection remains visible under hover.
`table-numeric` aligns to inline-end with tabular numerals; `table-center`,
`table-actions`, `table-checkbox`, `table-sort-icon` are shipped composition hooks.
Wrap wide native tables in `table-responsive`; do not turn cells into arbitrary
non-table elements without preserving table semantics. Label scrollable wrappers
and provide keyboard access where they overflow. Sorting, row selection and
navigation are application-owned, not automatic CSS behavior. Use native controls
inside cells and meaningful captions/header scopes. See [Indicator](/duskmoonui/docs/en/components/indicator/)
for label-plus-indicator cells.
...
- [Card](/duskmoonui/docs/en/components/card/) - Container for tables
- [Badge](/duskmoonui/docs/en/components/badge/) - Status indicators in tables
- [Button](/duskmoonui/docs/en/components/button/) - Action buttons in table cells
