...
Card has no default tilt effect. Compose statistics with `card-body` and `stats`;
see [Stat](/duskmoonui/docs/en/components/stat/) for a complete definition-list
example. Decorative motion requires the separate optional Hover 3D import.
