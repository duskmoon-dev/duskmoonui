...
Color variants use the selected role color as the background and its matching content color for readable text and controls.
...
Brand variants follow the active theme tokens. In the `moonlight` theme, primary is intentionally neutral while secondary carries the warm gold role; the trigger and Snackbar therefore use the same role color.
