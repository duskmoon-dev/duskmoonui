...
Canonical status classes are `avatar-status-online`, `avatar-status-offline`,
`avatar-status-busy`, `avatar-status-away`. `avatar-status` is a compatibility
composition marker; `avatar-online` / `avatar-offline` remain supported aliases.
Image/initials masking happens inside the Avatar; status remains unclipped.
Use visible labels, not color alone. A pseudo-element has no accessible name.

<ComponentShowcase title="Indicator composition" code={`<div class="avatar indicator avatar-lg avatar-ring avatar-ring-primary">
  <span class="avatar-placeholder">LM</span>
  <span class="indicator-item indicator-bottom badge badge-success badge-xs">Online</span>
</div><span>Luna Moon - Online</span>`} />

Use an `indicator-item` child OR the legacy pseudo-status class, not both. Indicator is in
the core aggregate; individual Avatar consumers must also import Indicator and Badge CSS.
...
<ComponentShowcase
  ...
  code={`<div class="avatar indicator">
  JD
</div>`}
/>
