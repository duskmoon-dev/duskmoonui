...
Skeletons reserve space while a loading region is being populated. Put `aria-busy="true"` on the loading region and hide decorative skeleton shapes with `aria-hidden="true"`; the CSS does not infer application loading state.
...
<ComponentShowcase title="Basic Skeleton" code={`<section aria-busy="true" aria-label="Loading profile">
...
- [Avatar](/duskmoonui/docs/en/components/avatar/) - User avatar component
- [Progress](/duskmoonui/docs/en/components/progress/) - Progress indicators
