...
@duskmoon-dev/core includes two carefully crafted themes. Full token values are listed in the [Themes](/duskmoonui/docs/en/theming/themes/) reference.

- [Themes reference](/duskmoonui/docs/en/theming/themes/) - Sunshine and moonlight tokens
...
Reference theme CSS variables directly. Token values are complete `oklch()` expressions — use them directly without wrapping:
...
/* With opacity using color-mix() — works regardless of token format */
.custom-overlay {
...
<div class="text-[var(--color-tertiary)]">
  Custom text color
</div>
```
...
@duskmoon-dev/core uses OKLCH color format generated from `@duskmoon-dev/design` YAML files:

```css
/* Tokens are complete oklch() values — use var() directly */
--color-primary: oklch(72% 0.17 75);
```

Use `color-mix()` for opacity and hover states:

```html
<div class="bg-primary">Full opacity primary</div>
```
...
/* Hover darkening — the pattern used internally by DuskMoonUI */
.btn-primary:hover {
  background-color: color-mix(in oklch, var(--color-primary), black 10%);
}
```

> **Why `color-mix(in oklch, ...)`?** OKLCH is a perceptually uniform color space — mixing colors in OKLCH produces more natural-looking transitions than mixing in sRGB or HSL. DuskMoonUI uses this for all hover/focus state calculations, regardless of the input color format.
