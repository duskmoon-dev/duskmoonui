...
Reference theme CSS variables directly. Token values are complete `hsl()` expressions — use them directly without wrapping:
...
@duskmoon-dev/core uses HSL color format generated from `@duskmoon-dev/design` YAML files:

```css
/* Tokens are complete hsl() values — use var() directly */
...
> **Why `color-mix(in oklch, ...)`?** OKLCH is a perceptually uniform color space — mixing colors in OKLCH produces more natural-looking transitions than mixing in sRGB or HSL. DuskMoonUI uses this for all hover/focus state calculations, even when the input token is in HSL format.
